...
def index():
    ...
    <form method="post">
        ...
        <select name="plan" required>
            ...
            <option value="Anual (1 año)">Anual (1 año)</option>
        </select>
        <button class="btn" type="submit">Registrar</button>
    </form>
    <form method="get" style="margin-top:10px;">
        <button class="btn" name="activos" value="1" type="submit">SOCIOS ACTIVOS</button>
        <a href="/" class="btn">Mostrar Todos</a>
    </form>
    <table>
        <tr>
        ...
        {% for d in dispositivos %}
        {% set status, color = estado_visual(d['status'], d['fecha_vencimiento']) %}
        <tr>
            <td>{{ d['hwid'] }}</td>
            <td>{{ d['nombre_socio'] or '' }}</td>
            <td>{{ d['plan'] or '' }}</td>
...
def validar(hwid):
    ...
    row = c.fetchone()
    conn.close()
    if row and row['status'] == 'aprobado' and row['fecha_vencimiento']:
        try:
            vencimiento = datetime.strptime(row['fecha_vencimiento'], '%Y-%m-%d').date()
            if datetime.now().date() <= vencimiento:
